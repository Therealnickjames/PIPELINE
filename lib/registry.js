...
function logEvent(config, sliceId, eventType, actor, details = {}, fromState = null, toState = null, meta = {}) {
  const db = getDb(config);
  db.prepare(`
    INSERT INTO events (slice_id, event_type, from_state, to_state, actor, details, created_at, run_id, request_id, correlation_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    sliceId,
    eventType,
    ...
    nowIso(),
    meta.runId || details.run_id || null,
    meta.requestId || details.request_id || null,
    meta.correlationId || details.correlation_id || null
  );
  writeStructuredLog(config, 'event', {
    slice_id: sliceId,
    ...
    to_state: toState,
    details,
    run_id: meta.runId || details.run_id || null,
    request_id: meta.requestId || details.request_id || null,
    correlation_id: meta.correlationId || details.correlation_id || null
  });
}
...
function transitionState(config, id, toState, actor, details = {}) {
  ...
  const transaction = db.transaction(() => {
    const latest = getRawSlice(config, id);
    if (!latest) {
      throw new PipelineError('SLICE_NOT_FOUND', `Unknown slice: ${id}`);
    }

    if (latest.status !== slice.status) {
      throw new PipelineError(
        'STALE_STATE',
        `Cannot transition ${id}; expected ${slice.status} but found ${latest.status}`,
        { sliceId: id, expected: slice.status, actual: latest.status }
      );
    }

    closeTiming(config, id, latest.status, now);

    const updateFields = {
      status: toState,
      agent_session_id: details.agent_session_id === undefined ? latest.agent_session_id : details.agent_session_id,
      branch_name: details.branch_name === undefined ? latest.branch_name : details.branch_name,
      last_signal: details.last_signal === undefined ? latest.last_signal ? toStoredJson(latest.last_signal) : null : toStoredJson(details.last_signal),
      needs_split: details.needs_split ? 1 : 0,
      split_reason: details.split_reason || null,
      last_request_id: details.request_id || latest.last_request_id || null,
      last_run_id: details.run_id || latest.last_run_id || null
    };

    if (toState === 'APPROVED') {
      updateFields.approved_at = latest.approved_at || now;
    }
    ...
    const assignments = Object.keys(updateFields).map((key) => `${key} = ?`);
    const values = [...Object.values(updateFields), now, id, latest.status];
    db.prepare(`
      UPDATE slices
    ...
    logEvent(config, id, 'state_change', actor, details, latest.status, toState, {
...
function markFailure(config, id, failureType, reason, actor, fromState, details = {}) {
  ...
  updateSlice(config, id, {
    ...
    last_request_id: details.request_id || slice.last_request_id || null,
    last_run_id: details.run_id || slice.last_run_id || null
  });
...
function resetForDispatch(config, id, branchName, sessionId, actor, details = {}) {
  ...
  updateSlice(config, id, {
    ...
    needs_split: 0,
    split_reason: null,
    last_request_id: details.request_id || current.last_request_id || null,
    last_run_id: details.run_id || current.last_run_id || null
  });
