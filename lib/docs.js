...
function scaffold(config) {
  ...
  const templates = {
    ...
    'fix-hypothesis.md': '# Fix Hypothesis\n\nNo cached or active hypothesis has been recorded yet.\n'
  };
...
function writeCurrentSlice(config, slice) {
  ...
  const content = [
    ...
    '',
    '## Failure Memory',
    '- No cached winning pattern applied yet.',
    '',
    '## Accuracy Gates',
    '- Write docs/preflight.md with all five required sections before editing non-doc files.',
    '- After basic slice tests pass, code coverage must be at least 90% and all mutation tests must pass before PR creation.',
    '',
    '## Do Not',
...
function writeFixHypothesis(config, content = null) {
  const body = content || '# Fix Hypothesis\n\nNo cached or active hypothesis has been recorded yet.\n';
  writeText(docsPath(config, 'fix-hypothesis.md'), `${body.trim()}\n`);
}

function appendCurrentSliceSection(config, title, lines) {
  const target = docsPath(config, 'current-slice.md');
  const current = readText(target, '# Current Slice\n');
  const block = [
    '',
    `## ${title}`,
    '',
  ...
  writeText(target, `${current.trimEnd()}${block}\n`);
}
